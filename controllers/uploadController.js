...
exports.handleUpload = async (req, res) => {
    try {
        ...
        // Transfer Detection & Matching
        const transferTransactions = csvService.detectTransferTransactions(
            uncategorizedTransactions
        );
        console.log("Detected transfer transactions:", transferTransactions);
        // Prepare system categorized records, AI categorized items & all transfers into 3 separate items to send to frontend
        // The user will check each set of data, then review it all together in one larger table & send their changes to the backend

        // Store final transaction records
